...
{
	if (Engine::Get_DIKeyState(DIK_A) == Engine::KEYDOWN) // аб
		m_pTransform->m_vInfo[INFO_POS].x -= 2.f;

	if (Engine::Get_DIKeyState(DIK_D) == Engine::KEYDOWN) // ©Л
		m_pTransform->m_vInfo[INFO_POS].x += 2.f;

	if (Engine::Get_DIKeyState(DIK_W) == Engine::KEYDOWN) // ╩С
		m_pTransform->m_vInfo[INFO_POS].y += 2.f;

	if (Engine::Get_DIKeyState(DIK_S) == Engine::KEYDOWN) // го
		m_pTransform->m_vInfo[INFO_POS].y -= 2.f;
}
